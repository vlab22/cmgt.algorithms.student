...
abstract class NodeGraph : Canvas
{
    ...
    private Brush _disabledNodeColor = Brushes.DimGray;
    ...
    protected virtual void drawNodes()
    {
        ...
        {
            drawNode(node, node.enabled ? _defaultNodeColor : _disabledNodeColor);
        }
    }
    ...
    protected virtual void drawNodeConnections(Node pNode)
    {
        ...
        {
            if (connection.enabled && pNode.enabled)
    ...
    public void ActivateNode(Node node, bool active)
    {
        if (nodes.Contains(node))
        {
            node.enabled = active;
